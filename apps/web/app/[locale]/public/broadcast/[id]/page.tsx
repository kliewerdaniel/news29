  ...
  useEffect(() => {
    ...
    const loadBroadcast = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(t("Broadcast.not_found"));
        }
        const data = await res.json();
        setBroadcast(data);
        trackBroadcastView(id as string);
      } catch (err) {
        setError(t("Broadcast.not_found"));
        console.error(err);
      }
    };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">{t("Broadcast.not_available")}</p>
        </div>
      </div>
    );
  }
            ...
            <div key={index} className="rounded-lg bg-white p-6 shadow-md">
              ...
              {segment.persona_comments && Object.entries(segment.persona_comments).length > 0 && (
                <div className="mt-4 space-y-3">
                  <h3 className="font-medium text-gray-900">{t("Broadcast.persona_perspectives")}:</h3>
